...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPaths = ["", ...Object.values(pages).map((page) => page.slug), `${indexBase}/compare`, `${indexBase}/methodology`, `${indexBase}/sources`, ...countriesAlphabetical.map((country) => `${indexBase}/${country.slug}`), "global-travel/dog-passport-planner", "global-travel/relocation", "global-travel/relocation-timeline", "global-travel/document-planning", "downloads/global-dog-owner-starter-guide", "lab/global-dog-cost-calculator", "lab/breed-fit-quiz", "lab/puppy-readiness-quiz", "lab/dog-travel-checklist", "lab/dog-age-calculator", "lab/dog-name-generator", ...academyTopics.map((topic) => `academy/${topic.slug}`), ...journalArticles.map((article) => `journal/${article.slug}`)];

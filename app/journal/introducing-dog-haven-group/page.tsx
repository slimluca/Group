...
function Article({ article }: { article: (typeof journalArticles)[number] }) {
  return (
    <>
      ...
      <section className="section"><div className="shell split"><div><p className="eyebrow">Dog Haven Group Journal</p><h1>{article.title}</h1><p className="lead">{article.description}</p></div><MediaFrame image={{ src: article.image, alt: "Dog Haven Group launch article visual with global platform planning materials." }} /></div></section>
      <section className="section editorial"><div className="shell split"><article>{article.body.map((p) => <p key={p}>{p}</p>)}<p>Continue into the <Link href="/doghaven-universe">Dog Haven Group Universe</Link>, <Link href="/countries">Country Network</Link>, or <Link href="/lab">Dog Haven Group Lab</Link> to explore the platform structure.</p></article><MediaFrame image={{ src: "/images/doghavengroup/journal/global-dog-newsroom.svg", alt: "Dog Haven Group Journal newsroom with global ownership, travel, country, and network planning boards." }} /></div></section>
    </>
  );
}

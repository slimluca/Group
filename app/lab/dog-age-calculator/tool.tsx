...
type LifeStage = "Puppy" | "Adolescent" | "Young adult" | "Mature adult" | "Senior";

const annualAgeRate: Record<DogSize, number> = { Small: 4, Medium: 5, Large: 6, Giant: 7 };
const seniorAge: Record<DogSize, number> = { Small: 10, Medium: 8, Large: 7, Giant: 6 };
const adolescentEnd: Record<DogSize, number> = { Small: 1.25, Medium: 1.5, Large: 1.75, Giant: 2 };
...
export default function DogAgeCalculator() {
  ...
  const result = useMemo(() => estimateDogAge(years, months, size), [years, months, size]);
  ...
  return (
    <section className="section">
      <div className="shell">
        ...
        <p className="lead">Estimate your dog’s approximate human-age equivalent and current life stage using age and body-size category. The result is an educational planning guide rather than a health or lifespan prediction.</p>
        <div className="tool">
          <div className="panel">
            <label className="field">
              <span>Dog age in years</span>
              <input type="number" min="0" max="30" step="1" value={years} onChange={(event) => setYears(Math.min(30, Math.max(0, Number(event.target.value))))} />
            </label>
            <label className="field">
              <span>Additional months (optional)</span>
              <input type="number" min="0" max="11" step="1" value={months} onChange={(event) => setMonths(Math.min(11, Math.max(0, Number(event.target.value))))} />
            </label>
            <label className="field">
            ...
            <div className="result-note">
              <span>This dog years calculator uses rapid early development and size-adjusted adult ageing. It does not rely on a universal “multiply by seven” rule.</span>
            </div>
          </div>
          <div className="panel" aria-live="polite">
            <p className="eyebrow">Estimated age-stage result</p>
            <div className="score">{result.range.low}–{result.range.high}</div>
            <h2>Human-age equivalent range</h2>
            <div className="result-grid">
              <div className="metric-card"><strong>{enteredAge}</strong><span>Entered dog age</span></div>
              <div className="metric-card"><strong>{result.stage}</strong><span>Estimated dog life stage</span></div>
            </div>
            <p>Dog size affects ageing patterns because smaller dogs often move through later adult years more gradually than large and giant dogs. Breed, genetics, health, body condition, environment, and veterinary history can all change how an individual dog ages.</p>
            <h3>Planning ideas for this stage</h3>
            <ul className="result-list">
              {result.suggestions.map((suggestion) => <li key={suggestion}>{suggestion}</li>)}
            </ul>
            <div className="result-note">
              <span>This estimate does not diagnose health, predict lifespan, make guarantees, or replace veterinary advice.</span>
            </div>
            <p>Continue with the <Link href="/academy/senior-dog-care">Senior Dog Care guide</Link>, explore the wider <Link href="/academy">Dog Haven Group Academy</Link>, or return to the <Link href="/lab">Dog Haven Group Lab</Link>.</p>
          </div>
        </div>
      </div>
    </section>
